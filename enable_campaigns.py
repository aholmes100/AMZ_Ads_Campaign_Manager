...
def enable_campaigns(access_token, profile_id, campaign_ids):
    for campaign_id in campaign_ids:
        ...
        payload = {
            "campaigns": [
                {"campaignId": campaign_id, "state": "ENABLED"}
                for campaign_id in campaign_ids
            ]
        }
        response = requests.put(url, headers=headers, data=json.dumps(payload))
        if response.status_code == 207:
            print(f"Successfully enalbled campaign {campaign_id}")
        else:
            print(f"Failed to enable campaign {campaign_id}: {response.status_code} - {response.text}")

campaign_ids_to_enable = [
...
if access_token:
    enable_campaigns(access_token, PROFILE_ID, campaign_ids_to_enable)